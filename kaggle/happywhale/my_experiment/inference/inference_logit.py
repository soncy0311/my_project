...
def mean_embeddings(emb_list, emb_size=2048,train=True,logit=False):
...
def inference(path, embed_path, n_neighbors=1000, threshold=0.5) :
    ...
    embed_mean = mean_embeddings(embed, logit=True)
    ...
    test_embed_mean = mean_embeddings(test_embed, train=False, logit=True)
